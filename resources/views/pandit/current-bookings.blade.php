...
@section('content')
    <div class="bg-white rounded-lg shadow overflow-hidden ">
        <nav class="flex space-x-6 text-gray-700 font-medium p-5">
            <a href="{{ route('pandit.current.bookings') }}"
                class="{{ request()->routeIs('pandit.current.bookings') ? 'text-[#FF6B00] font-semibold' : 'hover:text-[#FF6B00]' }}">
                Current Bookings
            </a>

            <a href="{{ route('pandit.previous.bookings') }}"
                class="{{ request()->routeIs('pandit.previous.bookings') ? 'text-[#FF6B00] font-semibold' : 'hover:text-[#FF6B00]' }}">
                Previous Bookings
            </a>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ceremony
                        </th>
                        <th scope="col"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($bookings as $booking)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 truncate max-w-xs" title="{{ $booking->location }}">
                                    {{ Str::limit($booking->location, 30) }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
...
@section('scripts')
    <script>
        document.getElementById('status-filter').addEventListener('change', function() {
            ...
            window.location.href = `{{ route('pandit.previous.bookings') }}?status=${status}`;
        });
    </script>
@endsection
